...
DEFINE_MOLDTEMP5_MAP

CONNECT_MOLDTEMP5_EVENT
...
#define TEMP_CTRL_FUNC    0x2f530010 //温控功能
#define TEMP_SEG_CNT      0x2f5300b4 //温度段数
#define TEMP_REAL_VALUE   0x2f560002 //温度实际值
#define TEMP_CTRL_STATUS  0x2f560012 //温控状态
#define MT_ROOM_TMP       0x2f560032 //室温的温度
...
static void SegFuncSelect()
{
    ...
    for(i = 0; i < 3; ++i)
    {
        ...
        {
            if(VarAdrToInt(d_TempCtrlMW5_MOLDSET_DATEMP_FUNC_MW5+i) != 0)//20220712 dyl
                VarSendSaveSetIntByAdr(d_TempCtrlMW5_MOLDSET_DATEMP_FUNC_MW5+i, 0);
        }
    }
}
...
static void ShowRoomTemp(MoldTemp5_PG* ppg)
{
...
static void ShowAlarm(MoldTemp5_PG* ppg)
{
...
static void SetValMax(UI16 seg)
{
    ...
    if(VarAdrToInt(d_TempCtrlMW5_MOLDSET_FL_TKTJ_MW0 + seg))//J型上限350
    {
         EditSetMax(pwgt, 350);
    }
    else//K型上限450
...
static int MoldTemp5OnInit(MoldTemp5_PG* ppg, PWGT pwgt, MSG_ID msg_id, int msg_value, int msg_para)
{
    return 0;
}
static int MoldTemp5OnShow(MoldTemp5_PG* ppg, PWGT pwgt, MSG_ID msg_id, int msg_value, int msg_para)
{
...
static int MoldTemp5OnHide(MoldTemp5_PG* ppg, PWGT pwgt, MSG_ID msg_id, int msg_value, int msg_para)
{
    SetPageLockMode(MODE_NOLOCK);//20210914 dyl touch
    return 0;
}
static int MoldTemp5OnUpdate(MoldTemp5_PG* ppg, PWGT pwgt, MSG_ID msg_id, int msg_value, int msg_para)
{
    TempRealShowUpdate();
    TempCtlStatusVal();
    ShowRoomTemp(ppg);
...
static int MoldTemp5OnChange(MoldTemp5_PG* ppg, PWGT pwgt, MSG_ID msg_id, int msg_value, int msg_para)
{
    UI32 index,addr;
    PATTR pattr;
    int i;
