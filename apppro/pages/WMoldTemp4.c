...
DEFINE_MOLDTEMP4_MAP

CONNECT_MOLDTEMP4_EVENT
...
#define TEMP_CTRL_FUNC    0x2f430010 //温控功能
#define TEMP_SEG_CNT      0x2f4300b4 //温度段数
#define TEMP_REAL_VALUE   0x2f460002 //温度实际值
#define TEMP_CTRL_STATUS  0x2f460012 //温控状态
#define MT_ROOM_TMP       0x2f460032 //室温的温度
...
static void SegFuncSelect()
{
    ...
    for(i = 0; i < 3; ++i)
    {
        ...
        {
            if(VarAdrToInt(d_TempCtrlMW4_MOLDSET_DATEMP_FUNC_MW5+i) != 0)//20220712 dyl
                VarSendSaveSetIntByAdr(d_TempCtrlMW4_MOLDSET_DATEMP_FUNC_MW5+i, 0);
        }
    }
}
...
static void ShowRoomTemp(MoldTemp4_PG* ppg)
{
...
static void ShowAlarm(MoldTemp4_PG* ppg)
{
...
static void SetValMax(UI16 seg)
{
    ...
    if(VarAdrToInt(d_TempCtrlMW4_MOLDSET_FL_TKTJ_MW0 + seg))//J型上限350
    {
         EditSetMax(pwgt, 350);
    }
    else//K型上限450
...
static int MoldTemp4OnInit(MoldTemp4_PG* ppg, PWGT pwgt, MSG_ID msg_id, int msg_value, int msg_para)
{
    return 0;
}
static int MoldTemp4OnShow(MoldTemp4_PG* ppg, PWGT pwgt, MSG_ID msg_id, int msg_value, int msg_para)
{
...
static int MoldTemp4OnHide(MoldTemp4_PG* ppg, PWGT pwgt, MSG_ID msg_id, int msg_value, int msg_para)
{
    SetPageLockMode(MODE_NOLOCK);//20210914 dyl touch
    return 0;
}
static int MoldTemp4OnUpdate(MoldTemp4_PG* ppg, PWGT pwgt, MSG_ID msg_id, int msg_value, int msg_para)
{
    TempRealShowUpdate();
    TempCtlStatusVal();
    ShowRoomTemp(ppg);
...
static int MoldTemp4OnChange(MoldTemp4_PG* ppg, PWGT pwgt, MSG_ID msg_id, int msg_value, int msg_para)
{
    UI32 index,addr;
    PATTR pattr;
    int i;
