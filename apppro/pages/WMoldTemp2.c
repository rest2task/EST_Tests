...
DEFINE_MOLDTEMP2_MAP

CONNECT_MOLDTEMP2_EVENT
...
#define TEMP_CTRL_FUNC    0x2f230010 //温控功能
#define TEMP_SEG_CNT      0x2f2300b4 //温度段数
#define TEMP_REAL_VALUE   0x2f260002 //温度实际值
#define TEMP_CTRL_STATUS  0x2f260012 //温控状态
#define MT_ROOM_TMP       0x2f260032 //室温的温度
...
static void SegFuncSelect()
{
    ...
    for(i = 0; i < 3; ++i)
    {
        ...
        {
            if(VarAdrToInt(d_TempCtrlMW2_MOLDSET_DATEMP_FUNC_MW5+i) != 0)//20220712 dyl
                VarSendSaveSetIntByAdr(d_TempCtrlMW2_MOLDSET_DATEMP_FUNC_MW5+i, 0);
        }
    }
}
...
static void ShowRoomTemp(MoldTemp2_PG* ppg)
{
...
static void ShowAlarm(MoldTemp2_PG* ppg)
{
...
static void SetValMax(UI16 seg)
{
    ...
    if(VarAdrToInt(d_TempCtrlMW2_MOLDSET_FL_TKTJ_MW0 + seg))//J型上限350
    {
         EditSetMax(pwgt, 350);
    }
    else//K型上限450
...
static int MoldTemp2OnInit(MoldTemp2_PG* ppg, PWGT pwgt, MSG_ID msg_id, int msg_value, int msg_para)
{
    return 0;
}
static int MoldTemp2OnShow(MoldTemp2_PG* ppg, PWGT pwgt, MSG_ID msg_id, int msg_value, int msg_para)
{
...
static int MoldTemp2OnHide(MoldTemp2_PG* ppg, PWGT pwgt, MSG_ID msg_id, int msg_value, int msg_para)
{
    SetPageLockMode(MODE_NOLOCK);//20210914 dyl touch
    return 0;
}
static int MoldTemp2OnUpdate(MoldTemp2_PG* ppg, PWGT pwgt, MSG_ID msg_id, int msg_value, int msg_para)
{
    TempRealShowUpdate();
    TempCtlStatusVal();
    ShowRoomTemp(ppg);
...
static int MoldTemp2OnChange(MoldTemp2_PG* ppg, PWGT pwgt, MSG_ID msg_id, int msg_value, int msg_para)
{
    UI32 index,addr;
    PATTR pattr;
    int i;
