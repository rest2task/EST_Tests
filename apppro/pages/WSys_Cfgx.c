...
static void G15SysCfgxUpdate()
{
    static UI32 time_100ms=0;
    static UI32 count = 0;
    PWGT pwgt_slstep;

    pwgt_slstep = PageGetWgtByName(sys_cfgx_pg_name,"slstep");
...
static int Sys_CfgxOnInit(Sys_Cfgx_PG* ppg, PWGT pwgt, MSG_ID msg_id, int msg_value, int msg_para)
{
    return 0;
}
static int Sys_CfgxOnShow(Sys_Cfgx_PG* ppg, PWGT pwgt, MSG_ID msg_id, int msg_value, int msg_para)
{
...
//#if 0
    ...
    if(/*IsG15Type()||*/IsTouchType())//20190629.cyx + //20220104 dyl
    {
        ...
        */
        ...
        WGTSetEnable(PageGetWgtByName(sys_cfgx_pg_name,"btcalibration"),FALSE);
        WGTSetEnable(PageGetWgtByName(sys_cfgx_pg_name,"edit114"),TRUE);//20220929 dyl
        WGTSetEnable(PageGetWgtByName(sys_cfgx_pg_name,"label58"),TRUE);//20220929 dyl
    }
    else
    {
        WGTSetEnable(PageGetWgtByName(sys_cfgx_pg_name,"btcalibration"),FALSE);//20220929 dyl
        WGTSetEnable(PageGetWgtByName(sys_cfgx_pg_name,"edit114"),FALSE);//20220929 dyl
        WGTSetEnable(PageGetWgtByName(sys_cfgx_pg_name,"label58"),FALSE);//20220929 dyl
    }
//#endif
...
static int Sys_CfgxOnHide(Sys_Cfgx_PG* ppg, PWGT pwgt, MSG_ID msg_id, int msg_value, int msg_para)
{
    SetPageLockMode(MODE_NOLOCK);//20210914 dyl touch
    return 0;
}
static int Sys_CfgxOnUpdate(Sys_Cfgx_PG* ppg, PWGT pwgt, MSG_ID msg_id, int msg_value, int msg_para)
{
...
static int Sys_CfgxOnChange(Sys_Cfgx_PG* ppg, PWGT pwgt, MSG_ID msg_id, int msg_value, int msg_para)
{
    UI32 addr,value;
    UI32 year,month,day;
...
//#endif
    ...
    case BRIGHTNESS_AUTO: //20191223.cyx
        ...
        if(VarAdrToUI16(BRIGHTNESS_AUTO))
        {
            ...
            WGTSetEnable(PageGetWgtByName(sys_cfgx_pg_name,"slstep"),FALSE);
        }
        else
        {
            WGTSetEnable(PageGetWgtByName(sys_cfgx_pg_name,"slstep"),TRUE);
    ...
    case CALIBRATE_PASSWORD:
        ...
        {
            WGTSetEnable(PageGetWgtByName(sys_cfgx_pg_name,"btcalibration"),TRUE);
        }
        else
        {
            WGTSetEnable(PageGetWgtByName(sys_cfgx_pg_name,"btcalibration"),FALSE);
    ...
    if(pwgt == PageGetWgtByName(sys_cfgx_pg_name,"btcalibration")) //20191223.cyx 触摸屏校验
    {
        char scmd[128];
