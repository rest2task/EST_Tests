...
DEFINE_MOLDTEMP1_MAP

CONNECT_MOLDTEMP1_EVENT
...
#define TMP_MTOPT_SHOW    TMPVAL(0)  //模温优化可见

#define TEMP_CTRL_FUNC    0x2f130010 //温控功能
#define TEMP_SET_VALUE    0x2f130000 //温度设定值
#define TEMP_PID_P        0x2f130020 //温度P系数
#define TEMP_PID_I        0x2f130030 //温度I系数
#define TEMP_PID_D        0x2f130040 //温度D系数
#define TEMP_HEAT_BUFF    0x2f130050 //温度加温缓冲区
#define TEMP_OUTPUT_CYCLE 0x2f130060 //温度输出周期时间
#define TEMP_SEG_CNT      0x2f1300b4 //温度段数
#define UPPER_DEVIATION   0x2f130070 //上偏差
#define LOWER_DEVIATION   0x2f130080 //下偏差
#define MT_FUNC           0x2f1300B6 //模温功能

#define TEMP_REAL_VALUE   0x2f160002 //温度实际值
#define TEMP_CTRL_STATUS  0x2f160012 //温控状态
#define MT_ROOM_TMP       0x2f160032 //室温的温度
...
static void SegFuncSelect()
{
    ...
    for(i = 0; i < 3; ++i)
    {
        ...
        {
            if(VarAdrToInt(d_TempCtrlMW1_MOLDSET_DATEMP_FUNC_MW5+i) != 0)//20220712 dyl
                VarSendSaveSetIntByAdr(d_TempCtrlMW1_MOLDSET_DATEMP_FUNC_MW5+i, 0);
        }
    }
}
...
static void TempOptEnable(MoldTemp1_PG* ppg,BOOL enable)
{
    WGTSetEnable(ppg->tabpage1_Tb4_btForceOpt,enable);
}
static void TempOptPageIn(MoldTemp1_PG* ppg)
{
    UI16 bit = 0;
    int i;

    for(i = 0; i < VarAdrToInt(MT_BOARD_COUNT); i++)
    {
        setone(bit, i);
    }
    VarAdrSetInt(TMP_MTOPT_SHOW, bit);

    VarAdrSetInt(TMP_TEMPOPT_PASSWORD,0);
    TempOptEnable(ppg,FALSE);
}
static int TempOptChange(UI32 addr, PWGT pwgt, MoldTemp1_PG* ppg)
{
    UI32 value;
    UI16 pdata[3]={0};
    ...
    case TMP_TEMPOPT_PASSWORD: //修改密码
        ...
        break;
    default:
        if(pwgt ==  ppg->tabpage1_Tb4_btOptimize) //优化
        {
            if(isMTOptimizex())//20200806
            {
                return 0;
            }
            ...
            {
                ShowMsg(VM_MSG_CLOSESPARE2);//20200812
                return 0;
            }

            if(VarAdrToInt(MT_OPT_SELECT) == 0)//20200806
            {
                ShowMsg(VM_MSG_SELMOLDTMP);
                return 0;
            }

            pdata[1] = VarAdrToInt(MT_OPT_SELECT);
            ProtSysCtrl(MT_OPTIMIZE_CMD,3,pdata);
            b_switchopt = FALSE;
        }
        else if(pwgt ==  ppg->tabpage1_Tb4_btForceOpt)//强制优化
        {
            if(isMTOptimizex())//20200806
            {
                return 0;
...
static void ShowRoomTemp(MoldTemp1_PG* ppg)
{
...
static void ShowAlarm(MoldTemp1_PG* ppg)
{
...
static void TempOptUpdate(MoldTemp1_PG* ppg)
{
    if(isMTOptimizex())
    {
        if(b_switchopt)
        ...
        WGTSetEnable(ppg->tabpage1_Tb4_btOptimize, FALSE);
        WGTSetEnable(ppg->tabpage1_Tb4_btForceOpt, FALSE);
    ...
    else
    {
        VarAdrSetStr(TMP_START_OPT, GetTextTran(TEXT_OPTIMIZE_START));
        VarAdrSetStr(TMP_START_FORCE, GetTextTran(TEXT_FORCE_OPTIMIZE_START));
        WGTSetEnable(ppg->tabpage1_Tb4_btOptimize, TRUE);
        if(VarAdrToUI16(TMP_TEMPOPT_PASSWORD) == OPT_PASSWORD)
        {
            WGTSetEnable(ppg->tabpage1_Tb4_btForceOpt, TRUE);
        }
        else
        {
            WGTSetEnable(ppg->tabpage1_Tb4_btForceOpt, FALSE);
...
static void SetValMax(UI16 seg)
{
    ...
    if(VarAdrToInt(d_TempCtrlMW1_MOLDSET_FL_TKTJ_MW0 + seg))//J型上限350
    {
         EditSetMax(pwgt, 350);
    }
    else//K型上限450
...
static int MoldTemp1OnInit(MoldTemp1_PG* ppg, PWGT pwgt, MSG_ID msg_id, int msg_value, int msg_para)
{

    return 0;
}
static int MoldTemp1OnShow(MoldTemp1_PG* ppg, PWGT pwgt, MSG_ID msg_id, int msg_value, int msg_para)
{
    ...
    if(GetCurrentUserId() < 2) //tab page show according to user privilege
    {
    ...
    for(i=0; i<MAX_MWSEG_CNT; i++) //mold temp. control related widget show
    {
        TempCtlFunc(i);
        SetValMax(i);
    }
...
static int MoldTemp1OnHide(MoldTemp1_PG* ppg, PWGT pwgt, MSG_ID msg_id, int msg_value, int msg_para)
{
    SetPageLockMode(MODE_NOLOCK);//20210914 dyl touch
    return 0;
}
static int MoldTemp1OnUpdate(MoldTemp1_PG* ppg, PWGT pwgt, MSG_ID msg_id, int msg_value, int msg_para)
{
    TempRealShowUpdate();
    TempCtlStatusVal();
    TempOptUpdate(ppg);
...
static int MoldTemp1OnChange(MoldTemp1_PG* ppg, PWGT pwgt, MSG_ID msg_id, int msg_value, int msg_para)
{
    UI32 index,addr;
    PATTR pattr;
    int i;
