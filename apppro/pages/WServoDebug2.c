...
DEFINE_SERVODEBUG2_MAP

CONNECT_SERVODEBUG2_EVENT
...
static UI8 ServoId[6] ={7,8,9,10,11,12};//驱动器的编号指针
...
static void GetServoCfg(PRO_SDO_RX_LOG* logs)
{
    ...
    EditSetText(servodebug2_pg.stMCU,cbuf);
    sprintf(cbuf,"%.1f",(logs+1)->servo_val/ 10.);//驱动FPGA软件版本号
    EditSetText(servodebug2_pg.stFPGA,cbuf);
    sprintf(cbuf,"%d",(logs+2)->servo_val);//驱动额定电压
    EditSetText(servodebug2_pg.stRatedVoltage,cbuf);
    sprintf(cbuf,"%.2f",(logs+3)->servo_val/ 100.);//驱动额定功率
    EditSetText(servodebug2_pg.stRatedPower,cbuf);
    sprintf(cbuf,"%.2f",(logs+4)->servo_val/ 100.);//驱动额定电流
    EditSetText(servodebug2_pg.stRatedCurrent,cbuf);
    ...
    EditSetText(servodebug2_pg.stMotorRatedVoltage,cbuf);
    sprintf(cbuf,"%.2f",(logs+6)->servo_val / 100.);//电机额定功率
    EditSetText(servodebug2_pg.stMotorPower,cbuf);
    sprintf(cbuf,"%.2f",(logs+7)->servo_val / 100.);//电机额定电流
    EditSetText(servodebug2_pg.stMotorRatedCurrent,cbuf);
    sprintf(cbuf,"%.2f",(logs+8)->servo_val / 100.);//电机额定扭矩
    EditSetText(servodebug2_pg.stRatedTorque,cbuf);
    sprintf(cbuf,"%.2f",(logs+9)->servo_val / 100.);//电机峰值扭矩
    EditSetText(servodebug2_pg.stMaxTorque,cbuf);
    sprintf(cbuf,"%d",(logs+10)->servo_val);//电机额定转速
    EditSetText(servodebug2_pg.stRatedRPM,cbuf);
    sprintf(cbuf,"%d",(logs+11)->servo_val);//电机最大转速
    EditSetText(servodebug2_pg.stMaxRPM,cbuf);
    sprintf(cbuf,"%d",(logs+12)->servo_val);//电机编码器类型
    EditSetText(servodebug2_pg.stEncodeType,cbuf);
...
static void Read_Callback(BOOL result, PRO_SDO_RX_LOG* logs, int cnt, void* param)
{
    ...
    {
        EditSetReadOnly(servodebug2_pg.stRotatedDir,FALSE);
        EditSetBKColor(servodebug2_pg.stRotatedDir,0xffffff);
        EditSetReadOnly(servodebug2_pg.stHighSpdGain,FALSE);
        EditSetBKColor(servodebug2_pg.stHighSpdGain,0xffffff);
        EditSetReadOnly(servodebug2_pg.stLowSpdGain,FALSE);
        EditSetBKColor(servodebug2_pg.stLowSpdGain,0xffffff);
        PageWgtSetFocus(servodebug2_pg.stRotatedDir);
        WGTSetVisible(servodebug2_pg.label2,FALSE);
        GetServoCfg(logs);
    }
}
...
static void ReqServoIdData(UI16 servo_id)
{
    ...
    EditSetReadOnly(servodebug2_pg.stRotatedDir,TRUE);
    EditSetBKColor(servodebug2_pg.stRotatedDir,0xdedede);
    EditSetReadOnly(servodebug2_pg.stHighSpdGain,TRUE);
    EditSetBKColor(servodebug2_pg.stHighSpdGain,0xdedede);
    EditSetReadOnly(servodebug2_pg.stLowSpdGain,TRUE);
    EditSetBKColor(servodebug2_pg.stLowSpdGain,0xdedede);
    WGTSetVisible(servodebug2_pg.label2,TRUE);
...
static void PageShow(ServoDebug2_PG* ppg)
{
    UI8 bit;
    int servo_num;
    bit = 0;
...
static int ServoDebug2OnInit(ServoDebug2_PG* ppg, PWGT pwgt, MSG_ID msg_id, int msg_value, int msg_para)
{
    int i;
    for(i = 0; i < SERVO_PAR_NO; ++i)
    {
...
static int ServoDebug2OnShow(ServoDebug2_PG* ppg, PWGT pwgt, MSG_ID msg_id, int msg_value, int msg_para)
{
    InitTempData();
//    ProtSysCtrl(REFRESH_SERVODATA,0,NULL);
    old_page = 0;
    ReqServoIdData(0);
...
static int ServoDebug2OnHide(ServoDebug2_PG* ppg, PWGT pwgt, MSG_ID msg_id, int msg_value, int msg_para)
{
    return 0;
}
static int ServoDebug2OnUpdate(ServoDebug2_PG* ppg, PWGT pwgt, MSG_ID msg_id, int msg_value, int msg_para)
{
    UI16 cur_page = TabPageGetCurSel(ppg->tabpage1);
    if(old_page != cur_page && !WGTGetVisible(servodebug2_pg.label2))
    {
...
static int ServoDebug2OnChange(ServoDebug2_PG* ppg, PWGT pwgt, MSG_ID msg_id, int msg_value, int msg_para)
{
    UI32 addr;
    PATTR pattr;
