...
DEFINE_SERVOPUMP2_MAP

CONNECT_SERVOPUMP2_EVENT

static UI16 old_page;
static QUESTION g_question;
static UI16 switch_key;
static BOOL bPump2Change = FALSE;
...
{
    if(key != GUI_KEY_FUN6 && VarAdrToInt(p_PP_MACHSET_ECAT_CHANGE_FLAG))
    {
...
static void PageShow(ServoPump2_PG* ppg)
{
    UI8 bit;
    int servo_num;
    BOOL isreadonly;
    ...
    if(servo_num>6) //第一页6个驱动，第二页显示从7开始
    {
        servo_num = servo_num-6;
    }
    ...
    if(VarAdrToInt(p_PP_SERVO_SERVO_CURRENT_NUM) < 8)
    {
    ...
    if(VarAdrToInt(d_machine1_MACHSET_FL_HYDRAUIC)&0x80)
    {
...
static void PageChangeShow(ServoPump2_PG* ppg)
{
    UI16 cur_page = TabPageGetCurSel(ppg->tabpage1);   
    WGTSetAttrByName(ppg->edit0, "text", d_Pump1_MACHSET_OIL_PUMP_DISPLACEMENT + (cur_page+6)*0x00100000);
    WGTSetAttrByName(ppg->edit1, "text", d_Pump1_MACHSET_PRESSAD_CH + (cur_page+6)*0x00100000);
    WGTSetAttrByName(ppg->edit2, "text", d_Pump1_MACHSET_MOTOR_SPEED_MAX + (cur_page+6)*0x00100000);
    WGTSetAttrByName(ppg->edit3, "text", d_Pump1_MACHSET_ENCODERESOLUTION + (cur_page+6)*0x00100000);
    WGTSetAttrByName(ppg->edit4, "text", d_Pump1_MACHSET_TORQUEMAX + (cur_page+6)*0x00100000);
    WGTSetAttrByName(ppg->edit5, "text", d_Pump1_MACHSET_BASEPRES + (cur_page+6)*0x00100000);
    WGTSetAttrByName(ppg->edit6, "text", d_Pump1_MACHSET_BASEFLOW + (cur_page+6)*0x00100000);
    WGTSetAttrByName(ppg->edit7, "text", d_Pump1_MACHSET_SPEED_ACC + (cur_page+6)*0x00100000);
    WGTSetAttrByName(ppg->edit8, "text", d_Pump1_MACHSET_SPEED_DEC + (cur_page+6)*0x00100000);
    WGTSetAttrByName(ppg->edit9, "text", d_Pump1_MACHSET_PRESS_ACC + (cur_page+6)*0x00100000);
    WGTSetAttrByName(ppg->edit10, "text", d_Pump1_MACHSET_PRESS_DEC + (cur_page+6)*0x00100000);
    WGTSetAttrByName(ppg->edit11, "text", d_Pump1_MACHSET_PRESS_ACC_FILTER + (cur_page+6)*0x00100000);
    WGTSetAttrByName(ppg->edit12, "text", d_Pump1_MACHSET_PRESS_DEC_FILTER + (cur_page+6)*0x00100000);
    WGTSetAttrByName(ppg->edit13, "text", d_Pump1_MACHSET_REVERSEFLOWMAX + (cur_page+6)*0x00100000);
    WGTSetAttrByName(ppg->edit14, "text", d_Pump1_MACHSET_TORQUEMAX_SLAVE_STOP + (cur_page+6)*0x00100000);
    WGTSetAttrByName(ppg->dropdown1, "value", d_Pump1_MACHSET_PRESSAD_TYPE + (cur_page+6)*0x00100000);
    WGTSetAttrByName(ppg->dropdown2, "value", d_Pump1_MACHSET_MODE_SLAVE_FOLLOW + (cur_page+6)*0x00100000);
}
...
static void ChangeMaxReverseSpd()
{
    ...
    {
        for(i = 6; i < VarAdrToInt(p_PP_SERVO_SERVO_CURRENT_NUM); ++i)
        {
        ...
        SavePumpPage(6, VarAdrToInt(p_PP_SERVO_SERVO_CURRENT_NUM));
    }
}
...
static int ServoPump2OnInit(ServoPump2_PG* ppg, PWGT pwgt, MSG_ID msg_id, int msg_value, int msg_para)
{
    return 0;
}
static int ServoPump2OnShow(ServoPump2_PG* ppg, PWGT pwgt, MSG_ID msg_id, int msg_value, int msg_para)
{
    old_page = 0;
    bPump2Change = FALSE;
...
static int ServoPump2OnHide(ServoPump2_PG* ppg, PWGT pwgt, MSG_ID msg_id, int msg_value, int msg_para)
{
...
static int ServoPump2OnUpdate(ServoPump2_PG* ppg, PWGT pwgt, MSG_ID msg_id, int msg_value, int msg_para)
{
    UI16 cur_page = TabPageGetCurSel(ppg->tabpage1);
    if(old_page != cur_page)
    {
        ImageSetIndex(ppg->image1, cur_page + 7);
        PageChangeShow(ppg);
        old_page = cur_page;
    }
    return 0;
}
static int ServoPump2OnChange(ServoPump2_PG* ppg, PWGT pwgt, MSG_ID msg_id, int msg_value, int msg_para)
{
    UI32 addr;
    PATTR pattr;
