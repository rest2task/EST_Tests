...
static void SetSeqVlvMode(UI32 offset,UI32 value)
{
    ...
    case 1:
        ...
        WGTSetAttrByName(pwgt_dlytm, "text", d_airblow1_MOLDSET_TM_SEQ_DELAY + 0x100000*(offset_n)+ 5*(offset_m)); // 0x23130007, 顺序阀1延时
        WGTSetAttrByName(pwgt_acttm, "text", d_airblow1_MOLDSET_TM_SEQ_MOVING + 0x100000*(offset_n)+ 5*(offset_m)); // 0x23130008, 顺序阀1动作时间
    ...
    case 2:
        WGTSetEnable(pwgt_dlytm, TRUE);
        WGTSetEnable(pwgt_acttm, TRUE);
        WGTSetAttrByName(pwgt_dlytm, "text", d_airblow1_MOLDSET_ADPOSI_SEQSTART + 0x100000*(offset_n)+ 5*(offset_m)); // 0x23130009, 顺序阀1动作起始位置
        WGTSetAttrByName(pwgt_acttm, "text", d_airblow1_MOLDSET_ADPOSI_SEQEND + 0x100000*(offset_n)+ 5*(offset_m)); // 0x2313000A, 顺序阀1动作结束位置
...
static void ShowVlvOutput()
{
    ...
    {
        i_n = i/3;i_m = i%3;
        VarAdrSetInt(TMP_VLV_NUM+i,VarAdrToInt(d_airblow1_PO_O_AIR_SEQVLV1+0x100000*i_n + i_m)); //顺序阀是3个为一小组
    }
}
...
static int Seq_VlvOnInit(Seq_Vlv_PG* ppg, PWGT pwgt, MSG_ID msg_id, int msg_value, int msg_para)
{
    return 0;
}
static int Seq_VlvOnShow(Seq_Vlv_PG* ppg, PWGT pwgt, MSG_ID msg_id, int msg_value, int msg_para)
{
...
static int Seq_VlvOnHide(Seq_Vlv_PG* ppg, PWGT pwgt, MSG_ID msg_id, int msg_value, int msg_para)
{
    SetPageLockMode(MODE_NOLOCK);//20210914 dyl touch
    return 0;
}
static int Seq_VlvOnUpdate(Seq_Vlv_PG* ppg, PWGT pwgt, MSG_ID msg_id, int msg_value, int msg_para)
{
    //20250108 fqh 顺序阀 刷新顺序阀的输出点状态
    ShowVlvOutput();
    return 0;
}
static int Seq_VlvOnChange(Seq_Vlv_PG* ppg, PWGT pwgt, MSG_ID msg_id, int msg_value, int msg_para)
{
