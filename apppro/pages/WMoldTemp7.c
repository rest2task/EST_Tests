...
DEFINE_MOLDTEMP7_MAP

CONNECT_MOLDTEMP7_EVENT
...
#define TEMP_CTRL_FUNC    0x2f730010 //温控功能
#define TEMP_SEG_CNT      0x2f7300b4 //温度段数
#define TEMP_REAL_VALUE   0x2f760002 //温度实际值
#define TEMP_CTRL_STATUS  0x2f760012 //温控状态
#define MT_ROOM_TMP       0x2f760032 //室温的温度
...
static void SegFuncSelect()
{
    ...
    for(i = 0; i < 3; ++i)
    {
        ...
        {
            if(VarAdrToInt(d_TempCtrlMW7_MOLDSET_DATEMP_FUNC_MW5+i) != 0)//20220712 dyl
                VarSendSaveSetIntByAdr(d_TempCtrlMW7_MOLDSET_DATEMP_FUNC_MW5+i, 0);
        }
    }
}
...
static void ShowRoomTemp(MoldTemp7_PG* ppg)
{
...
static void ShowAlarm(MoldTemp7_PG* ppg)
{
...
static void SetValMax(UI16 seg)
{
    ...
    if(VarAdrToInt(d_TempCtrlMW7_MOLDSET_FL_TKTJ_MW0 + seg))//J型上限350
    {
         EditSetMax(pwgt, 350);
    }
    else//K型上限450
...
static int MoldTemp7OnInit(MoldTemp7_PG* ppg, PWGT pwgt, MSG_ID msg_id, int msg_value, int msg_para)
{
    return 0;
}
static int MoldTemp7OnShow(MoldTemp7_PG* ppg, PWGT pwgt, MSG_ID msg_id, int msg_value, int msg_para)
{
...
static int MoldTemp7OnHide(MoldTemp7_PG* ppg, PWGT pwgt, MSG_ID msg_id, int msg_value, int msg_para)
{
    SetPageLockMode(MODE_NOLOCK);//20210914 dyl touch
    return 0;
}
static int MoldTemp7OnUpdate(MoldTemp7_PG* ppg, PWGT pwgt, MSG_ID msg_id, int msg_value, int msg_para)
{
    TempRealShowUpdate();
    TempCtlStatusVal();
    ShowRoomTemp(ppg);
...
static int MoldTemp7OnChange(MoldTemp7_PG* ppg, PWGT pwgt, MSG_ID msg_id, int msg_value, int msg_para)
{
    UI32 index,addr;
    PATTR pattr;
    int i;
