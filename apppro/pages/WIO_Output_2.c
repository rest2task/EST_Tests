...
DEFINE_IO_OUTPUT_2_MAP

CONNECT_IO_OUTPUT_2_EVENT

#define TMP_NAME0		TMPSTR(41)
#define TMP_CHECK0		TMPVAL(41)
#define PO_OUTPUT4      0x0616000D //输出点
...
static void Output__UpdateNames(PPROT_TBL_HD tbl_hd)
{
    ...
    for(i = 0; i < 16; ++i)
    {
        ...
        {
            if(tbl_hd && (pout = TableFindOutput(tbl_hd, i+GetSnNum(0))))
            {
                UI32 logic_addr = ((UI32)pout->part_id << 16) | pout->ind;

                if(pmap_output != NULL)
                {
                    snprintf(strid,sizeof(strid)-1,"%d(E%s)  %s",GetSnNum(i),GetExNum(i,buff),GetMapTran(pmap_output,logic_addr));
                }
                else
                {
            ...
            if((VarAdrToInt(SYS_FL_CTRLMODEL)==0x5000))
            {
                checkstate = VarAdrToInt(PO_OUTPUT4+1+(i/16))>>(i%16) & MASK_BIT0;
            }
            else
            {
                checkstate = VarAdrToInt(PO_OUTPUT4+(i/16))>>(i%16) & MASK_BIT0;
            }
...
static int IO_Output_2OnInit(IO_Output_2_PG* ppg, PWGT pwgt, MSG_ID msg_id, int msg_value, int msg_para)
{
    SetIOOut2ZeroCB(Force2Zero); //20190510 hz
    return 0;
}
static int IO_Output_2OnShow(IO_Output_2_PG* ppg, PWGT pwgt, MSG_ID msg_id, int msg_value, int msg_para)
{
...
static int IO_Output_2OnHide(IO_Output_2_PG* ppg, PWGT pwgt, MSG_ID msg_id, int msg_value, int msg_para)
{
...
static int IO_Output_2OnUpdate(IO_Output_2_PG* ppg, PWGT pwgt, MSG_ID msg_id, int msg_value, int msg_para)
{
...
static int IO_Output_2OnChange(IO_Output_2_PG* ppg, PWGT pwgt, MSG_ID msg_id, int msg_value, int msg_para)
{
	...
	if(index)
	{
        ...
        {
            ForceOutput(GetSnNum(index-1)-1);
        }
	}
    return 0;
}
