...
DEFINE_PUSHTABLE_MAP

CONNECT_PUSHTABLE_EVENT
...
static void LoadPushtabData()
{
	int i= 0;
	int index = 0;
	TRANSMETER_ITEM* pData=g_dbTransMeterTable;
...
static void WritePushPos(UI16 index)
{
	...
	{ //十字头位置UI16
        ...
        g_dbTransMeterTable[wIndex+PUSHTAB_PAGE_GROUPNUM*VarAdrToUI16(PUSHTABLE_PAGENUM)].wPump = wData;
        SavePushTableIndex(&g_dbTransMeterTable[wIndex+PUSHTAB_PAGE_GROUPNUM*VarAdrToUI16(PUSHTABLE_PAGENUM)], wIndex+PUSHTAB_PAGE_GROUPNUM*VarAdrToUI16(PUSHTABLE_PAGENUM));
	}
}
...
static void WriteMoldPos(UI16 index)
{
	...
	{ //模板位置UI16
        ...
        g_dbTransMeterTable[wIndex+PUSHTAB_PAGE_GROUPNUM*VarAdrToUI16(PUSHTABLE_PAGENUM)].wClamp = dwData;
        SavePushTableIndex(&g_dbTransMeterTable[wIndex+PUSHTAB_PAGE_GROUPNUM*VarAdrToUI16(PUSHTABLE_PAGENUM)], wIndex+PUSHTAB_PAGE_GROUPNUM*VarAdrToUI16(PUSHTABLE_PAGENUM));
	}
}
...
{
    WgtMsgConnect(pushtable_pg.button43, WGT_CLICK_MSG, button43OnClick);
    WgtMsgConnect(pushtable_pg.button42, WGT_CLICK_MSG, button42OnClick);
    WgtMsgConnect(pushtable_pg.button41, WGT_CLICK_MSG, button41OnClick);
}
static int PushtableOnInit(Pushtable_PG* ppg, PWGT pwgt, MSG_ID msg_id, int msg_value, int msg_para)
{
    return 0;
}
static int PushtableOnShow(Pushtable_PG* ppg, PWGT pwgt, MSG_ID msg_id, int msg_value, int msg_para)
{
...
static int PushtableOnHide(Pushtable_PG* ppg, PWGT pwgt, MSG_ID msg_id, int msg_value, int msg_para)
{
    return 0;
}
static int PushtableOnUpdate(Pushtable_PG* ppg, PWGT pwgt, MSG_ID msg_id, int msg_value, int msg_para)
{
	//udisk import
...
static int PushtableOnChange(Pushtable_PG* ppg, PWGT pwgt, MSG_ID msg_id, int msg_value, int msg_para)
{
	//UI16 value;
	UI16 index;
	UI32 addr; 
...
static int button41OnClick(PWGT pwgt, MSG_ID msg_id, int msg_value, int msg_para)
{
	...
	else
	{
        ...
        PanelRecAdd(pushtable_pg.button41, PNL_READ_U, 0);
	}
...
static int button42OnClick(PWGT pwgt, MSG_ID msg_id, int msg_value, int msg_para)
{
	...
	else
	{
		...
		VarAdrSetInt(UDISK_WR_PRIVILEGE,2); //导入导出 权限
        PanelRecAdd(pushtable_pg.button42, PNL_WRITE_U, 0);
	}
...
static int button43OnClick(PWGT pwgt, MSG_ID msg_id, int msg_value, int msg_para)
{
    ...
    PanelRecAdd(pushtable_pg.button43, PNL_RESET, 0);
