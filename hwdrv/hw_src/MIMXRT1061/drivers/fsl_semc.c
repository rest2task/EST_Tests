...
static status_t SEMC_IsIPCommandDone_Dm9k(SEMC_Type *base)
{
	unsigned long cnt = 0;
    /* Poll status bit till command is done*/
    while (!(base->INTR & SEMC_INTR_IPCMDDONE_MASK))
    {
		...
		if(cnt > 100)
		{
			PRINTF("SEMC_IsIPCommandDone_Dm9k  Err :  \r\n");
			break;
		}
    };
    /* Clear status bit */
...
status_t SEMC_ConfigureSRAM(SEMC_Type *base, semc_sram_config_t *config, uint32_t clkSrc_Hz)
{
  ...
  //  base->IOCR = iocReg | (muxCe << config->cePinMux);
    ...
    base->SRAMCR1 = SEMC_SRAMCR1_CES(0) |
                    SEMC_SRAMCR1_CEH(0) |
                    SEMC_SRAMCR1_AS(0) |
                    SEMC_SRAMCR1_AH(0) |
                    SEMC_SRAMCR1_WEL(15) |
    ...
    base->SRAMCR2 = SEMC_SRAMCR2_TA(0) |
                    SEMC_SRAMCR2_AWDH(0) |
                    SEMC_SRAMCR2_CEITV(0);
...
status_t SEMC_SendIPCommand_Dm9k(
    SEMC_Type *base, uint32_t address, uint16_t command, uint32_t write, uint32_t *read)
{
    uint32_t cmdMode;
    bool readCmd  = 0;
    bool writeCmd = 0;
    ...
    base->IPCMD = command | SEMC_IPCMD_KEY(SEMC_IPCOMMANDMAGICKEY);
    /* Wait for command done. */
    result = SEMC_IsIPCommandDone_Dm9k(base);
    if (result != kStatus_Success)
    {
        return result;
    }
 ...
 status_t SEMC_IPCommandNorRead(SEMC_Type *base, uint32_t address, uint8_t *data, uint32_t size_bytes)
{
 ...
 status_t SEMC_IPCommandNorRead_Dm9k(SEMC_Type *base, uint32_t address, uint8_t *data, uint32_t size_bytes)
{
    uint32_t tempData = 0;
    status_t result   = kStatus_Success;
 //   uint8_t dataSize  = base->NORCR0 & SEMC_NORCR0_PS_MASK; 
    {
   //     SEMC_ConfigureIPCommand(base, size_bytes);
        result = SEMC_SendIPCommand_Dm9k(base, address, kSEMC_NORDBICM_Read, 0, &tempData);
//        while (size_bytes)
//        {
//            size_bytes--;
//            *(data + size_bytes) = (tempData >> (SEMC_BYTE_NUMBIT * size_bytes)) & 0xFFU;
...
status_t SEMC_IPCommandNorWrite_Dm9k(SEMC_Type *base, uint32_t address, uint8_t *data, uint32_t size_bytes)
{
    uint32_t tempData = 0;
    status_t result   = kStatus_Success;
 //   uint8_t dataSize  = base->NORCR0 & SEMC_NORCR0_PS_MASK;
...
//        }
        ...
        result = SEMC_SendIPCommand_Dm9k(base, address, kSEMC_NORDBICM_Write, tempData, NULL);
    }
 //   SEMC_ConfigureIPCommand(base, dataSize);
