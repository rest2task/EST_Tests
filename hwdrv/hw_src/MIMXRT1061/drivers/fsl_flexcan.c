...
{
    if (CAN_CS_CODE(kFLEXCAN_TxMbInactive) == (base->MB[mbIdx].CS & CAN_CS_CODE_MASK))
    {
        return 1;
    }
        return 0;
...
//        (true == config->enableLoopBack) ? (base->CTRL1 | CAN_CTRL1_LPB_MASK) : (base->CTRL1 & ~CAN_CTRL1_LPB_MASK);
 ...
 //   mcrTemp = (true == config->enableIndividMask) ? (mcrTemp | CAN_MCR_IRMQ_MASK) : (mcrTemp & ~CAN_MCR_IRMQ_MASK);
    ...
    /* Enter Freeze Mode. */
    FLEXCAN_EnterFreezeMode(base);	//进入冻结模式
    base->MCR &= ~CAN_MCR_SRXDIS_MASK; //允许接收自身的帧
    base->MCR |= CAN_MCR_SRXDIS(1); //禁止接收自身的帧
    /* Exit Freeze Mode. */
    FLEXCAN_ExitFreezeMode(base); //退出冻结模式
}
