...
#define MPU6050_DEFAULT_ADDRESS 0x00
...
void lpi2c_master_callback(LPI2C_Type *base, lpi2c_master_handle_t *handle, status_t status, void *userData)
{
    ...
    {
        g_lm73.TEMP_NEW = true;
		g_lm73.WRITE_SUCCESS = true;
...
/**
  ...
  * @param   reg_add:寄存器地址
	* @param reg_data:要写入的数据
	*@param  intorno:写入方式，中断写入或等待延迟方式写入
  * @retval  
  */
uint32_t Sensor_write_hardware(uint8_t reg_add,uint8_t reg_dat,uint8_t intorno)
{ 
  lpi2c_master_transfer_t masterXfer = {0};
  status_t reVal = kStatus_Fail;
