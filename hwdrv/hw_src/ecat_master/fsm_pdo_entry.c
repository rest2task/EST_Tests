...
Description:  EtherCAT PDO entry config and access implementation

Others:

Function List:  
                    ec_fsm_pdo_entry_init: PDO entry Config and access structures init                   
                    ec_fsm_pdo_entry_clear: PDO entry Config and access structures clear(Free structures)
                    ec_fsm_pdo_entry_print: Print PDO entry info 
                    ec_fsm_pdo_entry_start_reading: Start read PDO entry config info 
                    ec_fsm_pdo_entry_start_configuration:Start PDO entry config
                    ec_fsm_pdo_entry_read_state_start: Start read PDO entry config info 
                    ec_fsm_pdo_entry_read_state_count: Read the number of PDO entry
                    ec_fsm_pdo_entry_read_action_next: Read next PDO entry config info
                    ec_fsm_pdo_entry_read_state_entry: Read PDO entry data
                    ec_fsm_pdo_entry_conf_state_start: Start Config PDO entry
                    ec_fsm_pdo_entry_conf_next_entry: Config next PDO entry
                    ec_fsm_pdo_entry_conf_state_zero_entry_count:Set the number of PDO entry to 0
                    ec_fsm_pdo_entry_conf_action_map: Start mapping new PDO entry
                    ec_fsm_pdo_entry_conf_state_map_entry: Start mapping new PDO entry state
                    ec_fsm_pdo_entry_conf_state_set_entry_count: Set new number of PDO entry
...
/******************************************************************************
 *
 *  $Id: fsm_pdo_entry.c,v 1.7 2015/02/28 01:09:50 kidd Exp $
 *
...
{
    _DBG_PRINTF_("Currently mapped PDO entries: ");
    ec_pdo_print_entries(fsm->cur_pdo);
    _DBG_PRINTF_(". Entries to map: ");
        ...
        )
{
    ...
    if (fsm->request.data_size != sizeof(uint32_t)) {
        ...
        if (!(pdo_entry = (ec_pdo_entry_t *)
                    ec_malloc(sizeof(ec_pdo_entry_t), 0))) {
            EC_SLAVE_ERR(fsm->slave, "Failed to allocate PDO entry.\n");
            fsm->state = ec_fsm_pdo_entry_state_error;
            return;
        }
#else
		pdo_entry = pdo_entry_pool_get();
		if (pdo_entry == NULL) 
		{
			EC_SLAVE_ERR(fsm->slave, "Failed to allocate PDO entry.\n");
			fsm->state = ec_fsm_pdo_entry_state_error;
