...
Description:  EtherCAT Slave config process
Others:
...
/******************************************************************************
 *
 *  $Id: slave_config.c,v 1.8 2015/02/28 01:09:50 kidd Exp $
 *
...
void ec_slave_config_init(
        ...
        uint16_t app_sc_id /**< APP config ID. */
        )
{
    unsigned int i;
        ...
        )
{
    ...
    list_for_each_entry_safe(req,ec_sdo_request_t, next_req, &sc->sdo_configs, list) {
        list_del(&req->list);
        ec_sdo_request_clear(req);
#ifdef DYN_MEM_ALLOC
        ec_free(req);
#else
		sdo_request_pool_put(req);
#endif
    }

    // free all SDO requests
    list_for_each_entry_safe(req, ec_sdo_request_t,next_req, &sc->sdo_requests, list) {
        list_del(&req->list);
        ec_sdo_request_clear(req);
#ifdef DYN_MEM_ALLOC
        ec_free(req);
    ...
    list_for_each_entry_safe(voe,ec_voe_handler_t, next_voe, &sc->voe_handlers, list) {
        list_del(&voe->list);
    ...
    list_for_each_entry_safe(soe, ec_soe_request_t,next_soe, &sc->soe_configs, list) {
        list_del(&soe->list);
        ...
        )
{
    ...
    {
        slave = sc->master->slaves;
        for(i = 0; i < sc->master->slave_count; i++)
        {
            if ((slave->sii.product_code == sc->product_code) && (slave->sii.vendor_id == sc->vendor_id)\
                && (slave->config == NULL))
            {
                break;
            }
    ...
    else
    {
        ...
        if (i == sc->master->slave_count) {
            EC_CONFIG_DBG(sc, 1, "SC with product code (0x%08X)"
                    " no slave to attach in current master.\n",
                    sc->product_code);
            return -EEXIST;
        }
    }
    ...
    sc->slave       = slave;
    //gonggw, 2014.11.24
    sc->alias       = slave->effective_alias;
        ...
        list_for_each_entry(pdo,ec_pdo_t, &sync_config->pdos.list, list) {
            list_for_each_entry(entry,ec_pdo_entry_t, &pdo->entries, list) {
                if (entry->index != index || entry->subindex != subindex) {
					...
					if (sync_config->dir == EC_DIR_OUTPUT)
					{
        ...
        uint8_t subindex, const uint8_t *data, size_t size)
{
	...
	_DBG_PRINTF_("%d in %s, ec_malloc size=%d\n", __LINE__, __FILE__, sizeof(ec_sdo_request_t));
#endif
    if (!(req = (ec_sdo_request_t *)
          ec_malloc(sizeof(ec_sdo_request_t), 0))) {
        EC_CONFIG_ERR(sc, "Failed to allocate memory for"
                " SDO configuration!\n");
        return -ENOMEM;
    }
#else
	req = sdo_request_pool_get();
    if (req == NULL) {
        EC_CONFIG_ERR(sc, "Failed to allocate memory for"
                " SDO configuration!\n");
        return -ENOMEM;
...
#ifdef EC_DYN_MEM_PRT
	_DBG_PRINTF_("%d in %s, ec_malloc size=%d\n", __LINE__, __FILE__, sizeof(ec_voe_handler_t));
#endif
    if (!(voe = (ec_voe_handler_t *)
                ec_malloc(sizeof(ec_voe_handler_t), 0))) {
        ...
        size_t size)
{
    ...
    if (!(req = (ec_soe_request_t *)
          ec_malloc(sizeof(ec_soe_request_t), 0))) {
        EC_CONFIG_ERR(sc, "Failed to allocate memory for"
                " IDN configuration!\n");
        return -ENOMEM;
    }
