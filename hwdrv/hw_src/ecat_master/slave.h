...
Description:  EtherCAT Slave definition
Others:
...
/******************************************************************************
 *
 *  $Id: slave.h,v 1.1 2015/02/06 05:23:05 kidd Exp $
 *
 ...
 */
#ifdef EC_DEBUG_SLAVE

#define EC_SLAVE_INFO(slave, fmt, args...)	\
	if(slave->master->debug_level > 1)		\
	    uart_printf("EtherCAT(%s) %u-%u: " fmt, __func__, slave->master->index, \
...
#define EC_SLAVE_ERR(slave, fmt, args...) \
    uart_printf("EtherCAT ERROR(%s) %u-%u: " fmt, __func__, slave->master->index, \
            slave->ring_position, ##args)
#else
#define EC_SLAVE_ERR(slave, fmt, args...)
#endif
...
#define EC_SLAVE_WARN(slave, fmt, args...) \
	if(slave->master->debug_level > 0)		\
	    uart_printf("EtherCAT WARNING(%s) %u-%u: " fmt, __func__, \
...
#define EC_SLAVE_DBG(slave, level, fmt, args...) \
    do { \
        if (slave->master->debug_level > 2) { \
            uart_printf("EtherCAT DEBUG(%s) %u-%u: " fmt, __func__, \
                    slave->master->index, slave->ring_position, ##args); \
        } \
    } while (0)
