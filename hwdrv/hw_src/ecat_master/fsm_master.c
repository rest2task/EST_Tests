...
Description:  EtherCAT Master FSM implementation

Others:

Function List:  
                    ec_fsm_master_init: Master FSM init                   
                    ec_fsm_master_clear: Master FSM clear(Free structures)
                    ec_fsm_master_exec: Master FSM executing 
                    ec_fsm_master_idle: return Idle of Master FSM   
                    ec_fsm_master_restart:Master FSM Restart
                    ec_fsm_master_state_start: Master FSM start
                    ec_fsm_master_state_broadcast: Send Broadcast datagram to probe the total ESC number in net work
                    ec_fsm_master_action_process_sii: access EEPROM in slave
                    ec_fsm_master_state_write_sii: write EEPROM
                    ec_fsm_master_action_process_register: access ESC's registers
                    ec_fsm_master_action_process_sdo: process SDO
                    ec_fsm_master_action_idle:Actions when  Master in Idle state
                    ec_fsm_master_action_next_slave_state: process next slave if exists
                    ec_fsm_master_action_configure: config slave
                    ec_fsm_master_state_read_state: read AL state
                    ec_fsm_master_state_acknowledge: master acknowledge
                    ec_fsm_master_state_clear_addresses: clear ESC's node address register
                    ec_fsm_master_state_scan_slave: scan slaves in network
                    ec_fsm_master_state_configure_slave: config slave
                    ec_fsm_master_enter_write_system_times: write system time to slaves
                    ec_fsm_master_dc_offset32: calculate DC offset in 32-bit mode
                    ec_fsm_master_dc_offset64:calculate DC offset in 64-bit mode
                    ec_fsm_master_state_dc_read_offset: Read time offset from slave
                    ec_fsm_master_state_dc_write_offset: write time offset to slave
                    ec_fsm_master_state_sdo_dictionary: access SDO dictionary
                    ec_fsm_master_state_sdo_request: Send SDO request
                    ec_fsm_master_state_reg_request: Access slave's registers
                    ec_master_sii_write_request_release: release SII access structures
                    ec_master_reg_request_release: release register access structures
                    ec_master_sdo_request_release: release SDO access structures
                    ec_master_foe_request_release: release FoE access structures
                    ec_master_soe_request_release: release SoE access structures
...
/******************************************************************************
 *
 *  $Id: fsm_master.c,v 1.7 2015/02/28 02:38:13 kidd Exp $
 *
...
/************gonggw, 2013.10.22************/
//the flag to set all slaves to INIT state
//int ec_fsm_slave_set;
//gonggw, 2013.10.23, set kthread_should_stop to 1 in ec_fsm_master_action_idle
extern u32 slave_cfg_finish;
/*****************************************************************************/
        ...
        )
{
	...
	if (fsm->link_state && !master->main_device.link_state) {
		...
		fsm->slave_states = (ec_slave_state_t)0x00;
		fsm->slaves_responding = 0; /* reset to trigger rescan on next link up. */
	}
	...
	if (fsm->slaves_responding) {
		...
		if (states != fsm->slave_states) { // slave states changed?
			...
			fsm->slave_states = (ec_slave_state_t)states;
...
#else
			master->slaves	=	&ec_slave_array[ec_slave_idx];
			ec_slave_idx	+= master->slave_count;
            if (ec_slave_idx > EC_SLAVE_NUM)
			{
                EC_MASTER_ERR(master, "Failed to allocate %u bytes"
                        " of slave memory!\n", size);
                master->slave_count = 0; // TODO avoid retrying scan!
...
#else
			ec_memcpy(fsm->datagram->data, request->data, request->length);
#endif
        }
        fsm->retries = EC_FSM_RETRIES;
...
#if 0 //gonggw, 2013.10.23, original code
    ec_fsm_master_restart(fsm);
#else
	fsm->state = ec_fsm_master_state_start; //set current state to ec_fsm_master_state_start, but not executed
	slave_cfg_finish = 1; //sall slaves init finish
#endif
}
        ...
        )
{
    ...
    ec_slave_set_state(slave, (ec_slave_state_t)EC_READ_U8(datagram->data));
        ...
        )
{
    ...
    if (slave->sii.mailbox_protocols & EC_MBOX_EOE) {
        ...
        ec_eoe_t *eoe;
#ifdef EC_DYN_MEM_PRT
		_DBG_PRINTF_("%d in %s, ec_malloc size=%d\n", __LINE__, __FILE__, sizeof(ec_eoe_t));
#endif
        if (!(eoe = ec_malloc(sizeof(ec_eoe_t), 0))) {
...
#ifdef EC_HAVE_CYCLES
	correction = (datagram->cycles_sent - slave->master->dc_cycles_app_start_time) * 1000000LL;
	do_div(&correction, cpu_khz);
#else
	correction = (u64) (((datagram->jiffies_sent - slave->master->dc_jiffies_app_start_time) * 1000000000) / HZ);
#endif

	if (slave->base_dc_range == EC_DC_32) {
		new_offset = ec_fsm_master_dc_offset32(fsm, system_time, old_offset, correction);
	}
	else
	{
 		new_offset = ec_fsm_master_dc_offset64(fsm, system_time, old_offset, correction);
	}
        ...
        )
{
	...
	if (datagram->state != EC_DATAGRAM_RECEIVED) {
		EC_SLAVE_ERR(slave, "Failed to receive DC system time offset datagram: ");
		ec_datagram_print_state(datagram);
		fsm->slave++;
		ec_fsm_master_enter_write_system_times(fsm);
		return;
...
#ifdef DYN_MEM_ALLOC
            if (request->data)
                ec_free(request->data);
#ifdef EC_DYN_MEM_PRT
			_DBG_PRINTF_("%d in %s, ec_malloc size=%d\n", __LINE__, __FILE__, request->length);
#endif
            request->data = (uint8_t *)ec_malloc(request->length, 0);
            if (!request->data) {
...
#else
		if (request->data)
			buffer_pool_put(request->data);
		request->data = (uint8_t *)buffer_pool_get(request->length);
		if (!request->data) {
			EC_MASTER_ERR(master, "Failed to allocate %zu bytes"
					" of memory for register data.\n", request->length);
			request->state = EC_INT_REQUEST_FAILURE;
...
#else
			ec_memcpy(request->data, datagram->data, request->length);
#endif
        }
 ...
 *
 */
void ec_master_sii_write_request_release(ec_sii_write_request_t * request)
{
    if (request->slave)
        EC_SLAVE_DBG(request->slave, 1, "Releasing SII write request %p.\n",
                request);
#ifdef DYN_MEM_ALLOC
    ec_free(request->words);
    ec_free(request);
#endif
}
/*****************************************************************************/
...
void ec_master_reg_request_release(ec_reg_request_t *request)
{
    if (request->slave)
 ...
 *
 */
void ec_master_sdo_request_release(ec_master_sdo_request_t *request)
{
    if (request->slave)
    ...
    ec_free(request);
#else
	master_sdo_request_pool_put(request);
#endif
}
/*****************************************************************************/
 ...
 *
 */
void ec_master_foe_request_release(ec_master_foe_request_t *request)
{
    if (request->slave)
 ...
 *
 */
void ec_master_soe_request_release(ec_master_soe_request_t *request)
{
    if (request->slave)
    ...
    ec_free(request);
#else
	master_soe_request_pool_put(request);
#endif
}
