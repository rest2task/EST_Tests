...
Description:  CoE(Canopen over EtherCAT) operation implementation

Others:

Function List:  
                    ec_canopen_abort_msg                               : Print Canopen message 
                    ec_fsm_coe_dictionary                                : Enter OD operation
                    ec_fsm_coe_transfer                                   : Start one CoE operation
                    ec_fsm_coe_exec                                        : CoE state executing
                    ec_fsm_coe_check_emergency                     : Check if emergency info
                    ec_fsm_coe_dict_start                                 : Start OD operation
                    ec_fsm_coe_dict_request                             : OD Operation Request
                    ec_fsm_coe_dict_check                                : Check the returned info of request 
                    ec_fsm_coe_dict_response                           : Send the Response info
                    ec_fsm_coe_dict_desc_request                     : Send read OD description request
                    ec_fsm_coe_dict_desc_check                        : Check the return info of descrition request
                    ec_fsm_coe_dict_entry_request                    : Send read OD entry request
                    ec_fsm_coe_dict_entry_check                       : Check the return info of OD entry request
                    ec_fsm_coe_dict_entry_response                  : Send OD entry response info
                    ec_fsm_coe_down_start                               : start CoE down(read) access
                    ec_fsm_coe_down_request                           : Send CoE down request datagram
                    ec_fsm_coe_down_check                              : Check the returned info of CoE down request datagram 
                    ec_fsm_coe_down_prepare_segment_request: Send Read data Seg request datagram
                    ec_fsm_coe_down_response                         : Send Response datagram
                    ec_fsm_coe_down_seg_request                     : Send data Seg request
                    ec_fsm_coe_down_seg_check                       : Check for data Seg request
                    ec_fsm_coe_down_seg_response                   : Send data Seg response
                    ec_fsm_coe_up_start                                   : start CoE up(write) access
                    ec_fsm_coe_up_request                               : Send CoE up request datagram
                    ec_fsm_coe_up_check                                  : Check the returned info of CoE up request datagram 
                    ec_fsm_coe_down_prepare_segment_request: Send Read data Seg request datagram
                    ec_fsm_coe_up_response                             : Send Response datagram
                    ec_fsm_coe_up_seg_request                        : Send data Seg request
                    ec_fsm_coe_up_seg_check                           : Check for data Seg request
                    ec_fsm_coe_up_seg_response                      : Send data Seg response
History:    
...
/******************************************************************************
 *
 *  $Id: fsm_coe.c,v 1.8 2015/02/28 02:38:13 kidd Exp $
 *
...
int watch_ch[10]={0};  //chenhao 2019.11.28
...
/*****************************************************************************/
u32 g_sdo_abort_code = 0;
u16 g_sdo_abort_servono = 0;
...
void ec_canopen_abort_msg(const ec_slave_t *slave, uint32_t abort_code)
{
	...
	g_sdo_abort_code = abort_code;
	g_sdo_abort_servono = slave->station_address;
...
void ec_fsm_coe_dict_response(ec_fsm_coe_t *fsm /**< finite state machine */)
{
    ...
    if (EC_READ_U16(data) >> 12 != 0x8 || // SDO information
        ...
        if (fsm->slave->master->debug_level>2) {
    ...
    for (i = 0; i < sdo_count; i++) {
		...
		_DBG_PRINTF_("%d in %s, ec_malloc size=%d\n", __LINE__, __FILE__, sizeof(ec_sdo_t));
#endif

        if (!(sdo = (ec_sdo_t *) ec_malloc(sizeof(ec_sdo_t), 0))) {
            EC_SLAVE_ERR(slave, "Failed to allocate memory for SDO!\n");
            fsm->state = ec_fsm_coe_error;
            return;
        }
#else
		sdo = (ec_sdo_t *)sdo_pool_get();
		if (sdo == NULL) {
            EC_SLAVE_ERR(slave, "Failed to allocate memory for SDO!\n");
            fsm->state = ec_fsm_coe_error;
            return;
...
#ifdef DYN_MEM_ALLOC
#ifdef EC_DYN_MEM_PRT
		_DBG_PRINTF_("%d in %s, ec_malloc size=%d\n", __LINE__, __FILE__, name_size + 1);
#endif
#ifdef CONFIG_USE_LIB_MEMCPY
        memcpy(sdo->name, data + 12, name_size);
#else
		ec_memcpy(sdo->name, data + 12, name_size);
#endif
#else
		if (name_size > (SDO_NAME_LEN-1))
			name_size = SDO_NAME_LEN-1;
		memcpy(sdo->name, data + 12, name_size);
#endif
...
#ifdef DYN_MEM_ALLOC
#ifdef EC_DYN_MEM_PRT
		_DBG_PRINTF_("%d in %s, ec_malloc size=%d\n", __LINE__, __FILE__, sizeof(ec_sdo_entry_t));
#endif
        if (!(entry = (ec_sdo_entry_t *)
                    ec_malloc(sizeof(ec_sdo_entry_t), 0))) {
            EC_SLAVE_ERR(slave, "Failed to allocate entry!\n");
            fsm->state = ec_fsm_coe_error;
            return;
        }
#else
		entry = (ec_sdo_entry_t *)sdo_entry_pool_get();
		if (entry == NULL) {
			EC_SLAVE_ERR(slave, "Failed to allocate entry!\n");
			fsm->state = ec_fsm_coe_error;
			return;
...
#ifdef DYN_MEM_ALLOC
#ifdef EC_DYN_MEM_PRT
			_DBG_PRINTF_("%d in %s, ec_malloc size=%d\n", __LINE__, __FILE__, data_size + 1);
#endif
            if (!(desc = (uint8_t *)ec_malloc(data_size + 1, 0))) {
...
#else
			ec_memcpy(entry->description, data + 16, data_size);
#endif
			entry->description[data_size] = 0;
#else
			if (data_size > (SDO_ENTRY_DESC_LEN-1))
				data_size = SDO_ENTRY_DESC_LEN-1;
			memcpy(entry->description, data + 16, data_size);
			entry->description[data_size] = 0;
#endif
        }
...
#else
		ec_memcpy(data + 6, request->data, request->data_size);
		ec_memset(data + 6 + request->data_size, 0x00, 4 - request->data_size);
#endif
...
#else
			ec_memcpy(data + EC_COE_DOWN_REQ_HEADER_SIZE,
#endif
...
#else
	ec_memcpy(data + EC_COE_DOWN_SEG_REQ_HEADER_SIZE,
			request->data + fsm->offset, segment_size);
#endif
    if (segment_size < EC_COE_DOWN_SEG_MIN_DATA_SIZE) {
#ifdef CONFIG_USE_LIB_MEMCPY
...
#else
		ec_memset(data + EC_COE_DOWN_SEG_REQ_HEADER_SIZE + segment_size, 0x00,
				EC_COE_DOWN_SEG_MIN_DATA_SIZE - segment_size);
#endif
    }
...
void ec_fsm_coe_up_request(ec_fsm_coe_t *fsm /**< finite state machine */)
{
    ...
    if (!ec_mbox_is_datagram_wc(mbox, 1)) {
        ...
        */
        ...
        printf("mbox->datagram->working_counter: %d \n\r",
                mbox->datagram->working_counter);
        
        
        printf("Reception of CoE upload request for"
                " SDO 0x%04x:%x failed with timeout after %lu ms: ",
                fsm->request->index, fsm->request->subindex, diff_ms);
    ...
    if (!ec_slave_mbox_check(mbox)) {
        ...
        if (diff_ms >= fsm->request->response_timeout) {
            ...
            EC_SLAVE_ERR(slave, "Timeout after %lu ms while waiting for"
                    " SDO 0x%04x:%x upload response, datagram state:%x:.\n", diff_ms,
                    fsm->request->index, fsm->request->subindex, mbox->datagram->state);
            return;
        }
...
#else
	ec_memcpy(request->data + request->data_size, data + 3, data_size);
#endif
