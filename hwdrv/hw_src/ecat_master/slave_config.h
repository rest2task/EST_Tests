...
Description:  EtherCAT Slave config definition
Others:
...
/******************************************************************************
 *
 *  $Id: slave_config.h,v 1.1 2015/02/06 05:23:05 kidd Exp $
 *
 ...
 */
#ifdef EC_DEBUG_SLAVE_CONFIG
#define EC_CONFIG_INFO(sc, fmt, args...) \
    uart_printf("EtherCAT(%s) %u %u:%u: " fmt, __func__, sc->master->index, \
...
#define EC_CONFIG_ERR(sc, fmt, args...) \
    uart_printf("in %s, EtherCAT ERROR %u %u:%u: " fmt, __func__, sc->master->index, \
            sc->alias, sc->position, ##args)
#else
#define EC_CONFIG_ERR(sc, fmt, args...)
#endif
...
#define EC_CONFIG_WARN(sc, fmt, args...) \
    uart_printf("EtherCAT WARNING(%s) %u %u:%u: " fmt, __func__, \
...
#define EC_CONFIG_DBG(sc, level, fmt, args...) \
    do { \
        if (sc->master->debug_level >= level) { \
            uart_printf("EtherCAT Slave DBG, %s %u %u:%u: " fmt, __func__, \
                    sc->master->index, sc->alias, sc->position, ##args); \
        } \
    } while (0)
#else
...
struct ec_slave_config {
    ...
    uint16_t            position;                           /**< Index after alias. If alias is zero, this is the ring position. */
    uint32_t            vendor_id;                          /**< Slave vendor ID. */
    uint32_t            product_code;                       /**< Slave product code. */

    uint16_t            watchdog_divider;                   /**< Watchdog divider as a number of 40ns intervals (see spec. reg. 0x0400). */
    uint16_t            watchdog_intervals;                 /**< Process data watchdog intervals (see  spec. reg. 0x0420). */
    uint8_t             allow_overlapping_pdos;             /**< Allow input PDOs use the same frame space as output PDOs. */
    ec_slave_t          *slave;                             /**< Slave pointer. This is \a NULL, if the slave is offline. */
    ...
    //gonggw, 2014.11.20
    uint16_t            app_sc_id;                          /** App SC id, to match with ID in APP */
