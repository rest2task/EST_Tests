...
Description:  EoE(Ethernet over EtherCAT) Application implementation

Others:

Function List:  
                    ec_eoe_init                :  EoE structures init
                    ec_eoe_clear              : EoE structures clear
                    ec_eoe_flush              :remove all TX frame in EoE TX queue
                    ec_eoe_send              : Generate EoE SDO package
                    ec_eoe_run                : Run EoE FSM
                    ec_eoe_queue            : Add datagram to EoE queue
                    ec_eoe_is_open          : Return the EoE Open State
                    ec_eoe_is_idle            : Return the EoE Idle State
                    ec_eoe_state_rx_start : Start one EoE RX
                    ec_eoe_state_rx_check: Check the last datagram is done
                    ec_eoe_state_rx_fetch : Send datagram to receive EoE data
                    ec_eoe_state_tx_sent  : Start one EoE TX
                    ec_eoedev_open          : Open EoE Device model
                    ec_eoedev_stop           : Stop EoE Device model
                    ec_eoedev_tx              : Add datagram EoE Tx queue
                    ec_eoedev_stats          : Return EoE device status
...
/******************************************************************************
 *
 *  $Id: ethernet.c,v 1.7 2015/02/28 02:38:13 kidd Exp $
 *
 ...
 */
#define EOE_DEBUG_LEVEL EC_DEBUG_LEVEL
...
//gonggw, 2015.02.14
#define EC_EOE_TX_FRAME_NUM	4 //TX frame pool
ec_eoe_frame_t eoe_tx_frame_array[EC_EOE_TX_FRAME_NUM];
#endif
/*****************************************************************************/
        ...
        )
{
	...
	for(i = 0; i < EC_EOE_TX_FRAME_NUM; i++)
		eoe_tx_frame_array[i].used = 0;
    eoe->slave = slave;
...
#else
	ec_memset(&eoe->stats, 0, sizeof(struct net_device_stats));
#endif
    eoe->rx_counter = 0;
    eoe->tx_counter = 0;
    eoe->rx_rate = 0;
    ...
    snprintf(eoe->datagram.name, EC_DATAGRAM_NAME_SIZE, name);
		eoe->dev = (struct net_device *)alloc_netdev(sizeof(ec_eoe_t *), name, ether_setup);
    if (eoe->dev == NULL) {
...
void ec_eoe_flush(ec_eoe_t *eoe /**< EoE handler */)
{
    ...
    list_for_each_entry_safe(frame, ec_eoe_frame_t,next, &eoe->tx_queue, queue) {
...
int ec_eoe_send(ec_eoe_t *eoe /**< EoE handler */)
{
    ...
    for (int i = 0; i < current_size; i++) {
        _DBG_PRINTF_("%02X ", eoe->tx_frame->skb->data[eoe->tx_offset + i]);
        if ((i + 1) % 16 == 0) {
            _DBG_PRINTF_("\n");
            EC_SLAVE_DBG(eoe->slave, 0, "");
        }
...
#else
	ec_memcpy(data + 4, eoe->tx_frame->skb->data + eoe->tx_offset, current_size);
#endif
 ...
 * datagram.
 */
#define RX_SKB_BUF_LEN	32
struct sk_buff rx_skb;
uint8_t rx_skb_buf[RX_SKB_BUF_LEN];
...
#ifdef DYN_MEM_ALLOC
        // new socket buffer
        if (!(eoe->rx_skb = (struct sk_buff *)dev_alloc_skb(fragment_offset * 32))) {
...
#else
	ec_memcpy(skb_put(eoe->rx_skb, data_size), data + 4, data_size);
#endif
                ...
                )
{
	...
	_DBG_PRINTF_("%d in %s, ec_malloc size=%d\n", __LINE__, __FILE__, sizeof(ec_eoe_frame_t));
#endif
    if (!(frame =
          (ec_eoe_frame_t *) ec_malloc(sizeof(ec_eoe_frame_t), 0))) {
	...
	for(i = 0; i < EC_EOE_TX_FRAME_NUM; i++)
	{
		if (eoe_tx_frame_array[i].used == 0)
		{
			frame = &eoe_tx_frame_array[i];
			frame->used = 1;
