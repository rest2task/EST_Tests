...
Description:  EtherCAT Slave process
Others:
...
/******************************************************************************
 *
 *  $Id: slave.c,v 1.13 2015/03/03 04:57:30 kidd Exp $
 *
...
#define SLAVE_DGRAM_DATA_LEN	EC_MAX_DATA_SIZE
uint8_t slave_dgram_data[SLAVE_DGRAM_DATA_LEN];
#endif
void ec_slave_init(
...
#else
	ec_memset(&slave->sii.coe_details, 0x00, sizeof(ec_sii_coe_details_t));
	ec_memset(&slave->sii.general_flags, 0x00, sizeof(ec_sii_general_flags_t));
#endif
...
#else
	slave->datagram.data = slave_dgram_data;
	slave->datagram.data_origin = EC_ORIG_EXTERNAL;
#endif
...
//    unsigned int i;
    ...
    // free all SDOs
    list_for_each_entry_safe(sdo,ec_sdo_t, next_sdo, &slave->sdo_dictionary, list) {
        list_del(&sdo->list);
        ec_sdo_clear(sdo);
    ...
    // free all SII PDOs
    list_for_each_entry_safe(pdo,ec_pdo_t, next_pdo, &slave->sii.pdos, list) {
        list_del(&pdo->list);
        ec_pdo_clear(pdo);
#ifdef DYN_MEM_ALLOC
        ec_free(pdo);
...
#ifdef DYN_MEM_ALLOC
#ifdef EC_DYN_MEM_PRT
		_DBG_PRINTF_("%d in %s, ec_malloc size=%d\n", __LINE__, __FILE__, sizeof(char *) * slave->sii.string_count);
#endif
        if (!(slave->sii.strings =
                    (char **)ec_malloc(sizeof(char *) * slave->sii.string_count,
                        0))) {
...
#else
		if ((slave->sii.string_count > MAX_STRING_PER_SLAVE) || sii_string_idx > (EC_SLAVE_NUM))
		{
			EC_SLAVE_ERR(slave, "Failed to allocate string array memory(string count is %d, slaveNO=%d).\n", slave->sii.string_count, sii_string_idx);
			err = -ENOMEM;
			goto out_zero;
		}
		slave->sii.strings = sii_string_ptr;
		sii_string_idx++;
#endif
        offset = 1;
        for (i = 0; i < slave->sii.string_count; i++) {
...
#ifdef EC_DYN_MEM_PRT
			_DBG_PRINTF_("%d in %s, ec_malloc size=%d\n", __LINE__, __FILE__, sizeof(char) * size + 1);
#endif
            if (!(slave->sii.strings[i] =
                        (char *)ec_malloc(sizeof(char) * size + 1, 0))) {
                EC_SLAVE_ERR(slave, "Failed to allocate string memory.\n");
                err = -ENOMEM;
                goto out_free;
            }
#else
	//	if (!(slave->sii.strings[i] =no_free_buf_get())) {
	slave->sii.strings[i] = (char *)no_free_buf_get(size+1);
	if (slave->sii.strings[i] == NULL) {
				EC_SLAVE_ERR(slave, "Failed to allocate string memory.\n");
				err = -ENOMEM;
				goto out_free;
}
...
#ifdef EC_DYN_MEM_PRT
		_DBG_PRINTF_("%d in %s, ec_malloc size=%d\n", __LINE__, __FILE__, memsize);
#endif
        if (!(syncs = (ec_sync_t *)ec_malloc(memsize, 0))) {
            EC_SLAVE_ERR(slave, "Failed to allocate %zu bytes"
                    " for sync managers.\n", memsize);
            return -ENOMEM;
        }
#else
		if ((sii_sync_idx > (EC_SLAVE_NUM - 1)) || (total_count > EC_MAX_SYNC_MANAGERS)) {
			EC_SLAVE_ERR(slave, "Failed to allocate %zu bytes"
					" for sync managers.\n", memsize);
			return -ENOMEM;
		}
		syncs = sii_sync_array[sii_sync_idx];
		sii_sync_idx++;
#endif
        for (i = 0; i < slave->sii.sync_count; i++)
        ...
        )
{
    ...
    while (data_size >= 8) {
...
#endif
        if (!(pdo = (ec_pdo_t *)ec_malloc(sizeof(ec_pdo_t), 0))) {
        ...
        printf("\n\r chen---PDO index=0x%x(sync=%d), entry count = %d\n\r", pdo->index, pdo->sync_index, entry_count);
        
		//gonggw, 2015.03.03, return if invalid PDO info(after invalid sync index)
		if ((pdo->sync_index < 0) || (pdo->sync_index >= EC_MAX_SYNC_MANAGERS))
		{
			pdo_pool_put(pdo);
			return 0;
		}
        ...
        data += 8;
#if 1
		_DBG_PRINTF_("%d in %s, PDO index=0x%x(sync=%d), entry count = %d\n\r", __LINE__, __FILE__, \
			pdo->index, pdo->sync_index, entry_count);
#endif

        for (i = 0; i < entry_count; i++) {
#ifdef DYN_MEM_ALLOC
#ifdef EC_DYN_MEM_PRT
			_DBG_PRINTF_("%d in %s, ec_malloc size=%d\n", __LINE__, __FILE__, sizeof(ec_pdo_entry_t));
#endif
            if (!(entry = (ec_pdo_entry_t *)ec_malloc(sizeof(ec_pdo_entry_t), 0))) {
                EC_SLAVE_ERR(slave, "Failed to allocate PDO entry memory.\n");
                return -ENOMEM;
            }
#else
			entry = pdo_entry_pool_get();
			if (entry == NULL) {
				EC_SLAVE_ERR(slave, "Failed to allocate PDO entry memory.\n");
				return -ENOMEM;
			}
