...
Description:  SoE access FSM implementation

Others:

Function List:  
                    ec_print_soe_error: Print SoE Error info                
                    ec_fsm_soe_init: SoE FSM structures init                   
                    ec_fsm_soe_clear:  
                    ec_fsm_soe_transfer: SoE Access entry
                    ec_fsm_soe_exec:Executing SoE state function
                    ec_fsm_soe_success: SoE success flag
                    ec_fsm_soe_print_error: SoE FSM error print
                    ec_fsm_soe_read_request: SoE Read Reqest
                    ec_fsm_soe_read_check: Check the SoE Read status
                    ec_fsm_soe_read_response: Return SoE Response data
                    ec_fsm_soe_write_next_fragment: Send datagram of writing next part of data 
                    ec_fsm_soe_write_start: Start SoE write Request
                    ec_fsm_soe_write_request: Generate SoE write request datagram
                    ec_fsm_soe_write_check: Check the SoE Write status
                    ec_fsm_soe_write_response: Return SoE Response data
                    ec_fsm_soe_error: SoE access error flag
                    ec_fsm_soe_end: SoE access success flag
...
/******************************************************************************
 *
 *  $Id: fsm_soe.c,v 1.1 2015/02/06 05:23:05 kidd Exp $
 *
...
void ec_fsm_soe_print_error(ec_fsm_soe_t *fsm /**< Finite state machine */)
{
    ...
    if (request->dir == EC_DIR_OUTPUT) {
        _DBG_PRINTF_("Writing");
    } else {
        _DBG_PRINTF_("Reading");
    }

    _DBG_PRINTF_(" IDN 0x%04X failed.\n", request->idn);
}
...
#else
	ec_memcpy(data + 4, req->data + fsm->offset, fragment_size);
#endif
