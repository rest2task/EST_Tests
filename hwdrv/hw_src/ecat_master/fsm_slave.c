...
Description:  main slave FSM implementation

Others:

Function List:  
                    ec_fsm_slave_init: Main slave FSM structures init                   
                    ec_fsm_slave_clear: Main slave FSM structures clear 
                    ec_fsm_slave_ready: Return slave ready
                    ec_fsm_slave_state_ready:Operations when slave is in ready state
                    ec_fsm_slave_action_process_sdo: Start to process SDO 
                    ec_fsm_slave_state_sdo_request: Start to process SDO statet
                    ec_fsm_slave_action_process_foe: Start to process FOE 
                    ec_fsm_slave_state_foe_request: Start to process FOE statet
                    ec_fsm_slave_action_process_soe: Start to process SOE 
                    ec_fsm_slave_state_soe_request: Start to process SOE statet
...
/******************************************************************************
 *
 *  $Id: fsm_slave.c,v 1.1 2015/02/06 05:23:05 kidd Exp $
 *
        ...
        )
{
    ...
    list_for_each_entry_safe(request,ec_master_sdo_request_t, next, &slave->slave_sdo_requests, list) {
        ...
        )
{
    ...
    list_for_each_entry_safe(request, ec_master_foe_request_t,next, &slave->foe_requests, list) {
        if (slave->current_state & EC_SLAVE_STATE_ACK_ERR) {
        ...
        )
{
    ...
    // search the first request to be processed
    list_for_each_entry_safe(request,ec_master_soe_request_t, next, &slave->soe_requests, list) {
