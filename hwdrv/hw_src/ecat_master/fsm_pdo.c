...
Description:  EtherCAT PDO config and access implementation

Others:

Function List:  
                    ec_fsm_pdo_init: PDO Config and access structures init                   
                    ec_fsm_pdo_clear: PDO Config and access structures clear(Free structures)
                    ec_fsm_pdo_print: Print PDO info 
                    ec_fsm_pdo_start_reading: Start read PDO config info 
                    ec_fsm_pdo_start_configuration:Start PDO config
                    ec_fsm_pdo_read_state_start: Start read PDO config info 
                    ec_fsm_pdo_read_action_next_sync: Config next PDO
                    ec_fsm_pdo_read_state_pdo_count: Read PDO entry number
                    ec_fsm_pdo_read_action_next_pdo: Read next PDO config
                    ec_fsm_pdo_read_state_pdo: Read PDO config info
                    ec_fsm_pdo_read_state_pdo_entries: Start PDO entries info
                    ec_fsm_pdo_conf_state_start:Start PDO config
                    ec_fsm_pdo_conf_action_next_pdo: Config next PDO
                    ec_fsm_pdo_conf_action_next_sync: Config next PDO entries
                    ec_fsm_pdo_conf_state_read_mapping: Re-config the PDOs
                    ec_fsm_pdo_conf_action_check_mapping: Check the PDO Remapping result
                    ec_fsm_pdo_conf_state_mapping: Start PDO mapping
                    ec_fsm_pdo_conf_action_next_pdo_mapping: Start Next PDO mapping
                    ec_fsm_pdo_conf_action_check_assignment: 
                    ec_fsm_pdo_conf_state_zero_pdo_count: set PDO count to 0(this is necessary before PDO re-config)
                    ec_fsm_pdo_conf_action_assign_pdo: Assign new PDO
                    ec_fsm_pdo_conf_state_assign_pdo:Assign new PDO state
                    ec_fsm_pdo_conf_state_set_pdo_count: Set new PDO count
...
/******************************************************************************
 *
 *  $Id: fsm_pdo.c,v 1.7 2015/02/28 02:38:13 kidd Exp $
 *
...
{
    _DBG_PRINTF_("Currently assigned PDOs: ");
    ec_pdo_list_print(&fsm->sync->pdos);
    _DBG_PRINTF_(". PDOs to assign: ");
        ...
        )
{
...
#if 0
    {
        ...
        uint8_t sync_index = slave->sii.sync_count;
        while(sync_index > 1)
        {
            sync_index--;
            _DBG_PRINTF_("sync_index=%x:\n", sync_index);
            list_head = &(slave->sii.syncs[sync_index].pdos.list);
            temp_list = list_head->next;
            while(temp_list != list_head)
            {
                pdo = list_entry(temp_list->next, typeof(*pdo), list);
                _DBG_PRINTF_("pdo is %p\n", pdo);
        ...
        )
{
    ...
    if ((fsm->slave->sii.mailbox_protocols & EC_MBOX_COE)
        ...
        EC_SLAVE_WARN(fsm->slave, "");
		EC_SLAVE_INFO(fsm->slave, "Currently mapped PDO entries: ");
		ec_pdo_print_entries(&fsm->slave_pdo);
		EC_SLAVE_INFO(fsm->slave, ". Entries to map: ");
		ec_pdo_print_entries(fsm->pdo);
		EC_SLAVE_INFO(fsm->slave, "\n");
    }
