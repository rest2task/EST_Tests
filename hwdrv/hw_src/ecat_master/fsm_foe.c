...
/******************************************************************************
 *
 *  $Id: fsm_foe.c,v 1.1 2015/02/06 05:23:04 kidd Exp $
 *
...
#else
	ec_memcpy(data + EC_FOE_HEADER_SIZE,
			fsm->tx_buffer + fsm->tx_buffer_offset, current_size);
#endif
...
#else
	ec_memcpy(data + EC_FOE_HEADER_SIZE, fsm->tx_filename, current_size);
#endif
    return 0;
}
...
#ifdef DEBUG_FOE
    _DBG_PRINTF_("ec_fsm_foe_write_start()\n");
#endif
...
#ifdef DEBUG_FOE
    _DBG_PRINTF_("ec_foe_state_sent_wrq()\n");
#endif
        ...
        )
{
...
#ifdef DEBUG_FOE
    _DBG_PRINTF_("ec_fsm_foe_ack_read()\n");
#endif
    ...
    if (!ec_mbox_is_datagram_wc(mbox, 1)) {
		...
		data = ec_slave_mbox_fetch(fsm->slave, fsm->mbox,&mbox_prot, &rec_size);
    if (data == NULL) {
        ec_foe_set_tx_error(fsm, FOE_PROT_ERROR);
        return;
    }
...
#ifdef DEBUG_FOE
    _DBG_PRINTF_("ec_fsm_foe_state_data_sent()\n");
#endif
...
#ifdef DEBUG_FOE
    _DBG_PRINTF_("ec_fsm_foe_ack_check()\n");
#endif
...
#else
	ec_memcpy(data + EC_FOE_HEADER_SIZE, fsm->rx_filename, current_size);
#endif
...
#ifdef DEBUG_FOE
    _DBG_PRINTF_("ec_fsm_foe_read_start()\n");
#endif
...
#ifdef DEBUG_FOE
    _DBG_PRINTF_("ec_foe_state_rrq_sent()\n");
#endif
...
#ifdef DEBUG_FOE
    _DBG_PRINTF_("ec_fsm_foe_state_data_check()\n");
#endif
        ...
        )
{
...
#ifdef DEBUG_FOE
    _DBG_PRINTF_("ec_fsm_foe_state_data_read()\n");
#endif
    ...
    if (opCode == EC_FOE_OPCODE_ERR) {
        ...
        if (rec_size > 6) {
            uint8_t text[256];
            strncpy((char *)text, (const char*)(data + 6), min(rec_size - 6, sizeof(text)));
...
#else
		ec_memcpy(fsm->rx_buffer + fsm->rx_buffer_offset,
				data + EC_FOE_HEADER_SIZE, rec_size);
#endif
    ...
    if (fsm->rx_last_packet ||
        ...
        _DBG_PRINTF_ ("ERROR: data doesn't fit in receive buffer\n");
        _DBG_PRINTF_ ("       rx_buffer_size  = %d\n", fsm->rx_buffer_size);
        _DBG_PRINTF_ ("       rx_buffer_offset= %d\n", fsm->rx_buffer_offset);
        _DBG_PRINTF_ ("       rec_size        = %zd\n", rec_size);
        _DBG_PRINTF_ ("       rx_mailbox_size = %d\n",
                slave->configured_rx_mailbox_size);
        _DBG_PRINTF_ ("       rx_last_packet  = %d\n", fsm->rx_last_packet);
...
#ifdef DEBUG_FOE
    _DBG_PRINTF_("ec_foe_state_sent_ack()\n");
#endif
