...
Description:  Functions for change ESC's state

Others:

Function List:  
                    ec_fsm_change_init                   : FSM change structure init
                    ec_fsm_change_start                 : Start FSM change
                    ec_fsm_change_ack                   : Set FSM change to Acknoledge mode
                    ec_fsm_change_exec                 : FSM change state executing
                    ec_fsm_change_success             : FSM change success
                    ec_fsm_change_state_start        : FSM change start state
                    ec_fsm_change_state_check       : Read current state before state change
                    ec_fsm_change_state_status      : Check current state before state change 
                    ec_fsm_change_state_start_code: Read Status Code when state erro
                    ec_fsm_change_state_ack          : FSM change state ACK
                    ec_fsm_change_state_check_ack: Check the FSM change ACK
History:    
...
/******************************************************************************
 *
 *  $Id: fsm_change.c,v 1.3 2015/02/28 08:35:28 kidd Exp $
 *
                                ...
                                /**< finite state machine */)
{
...
#if 1 //gonggw, 2015.01.27, original code
    if (slave->current_state != fsm->old_state) { // state changed
#else
    if ((slave->current_state != fsm->old_state) || (slave->current_state & 0x10)) { // state changed
#endif
    ...
    if (datagram->jiffies_received - fsm->jiffies_start >=
        ...
        ec_state_string(fsm->requested_state, state_str, 0);
        fsm->state = ec_fsm_change_state_error;
        EC_SLAVE_ERR(slave, "Timeout while setting state %s(r=%x, s=%x).\n", state_str, datagram->jiffies_received, fsm->jiffies_start);
        return;
    }
