...
Description:  SII access implementation

Others:

Function List:  
                    ec_fsm_sii_init: SII access structures init                   
                    ec_fsm_sii_read: SII read entry 
                    ec_fsm_sii_write: SII write entry  
                    ec_fsm_sii_state_start_reading:Start SII read state
                    ec_fsm_sii_state_read_check: SII read check 
                    ec_fsm_sii_state_read_fetch: Read the EEPROM data
                    ec_fsm_sii_state_start_writing:Start SII write state
                    ec_fsm_sii_state_write_check: SII write request check
                    ec_fsm_sii_state_write_check2: SII write request check2
...
/******************************************************************************
 *
 *  $Id: fsm_sii.c,v 1.2 2015/02/28 02:38:13 kidd Exp $
 *
        ...
        )
{
    ...
    //gonggw, 2014.03.17, for supporting 8 byte reading
    data_size = (fsm->length > 4) ? 8 : 4;
    data_size += 6;
    ...
    switch (fsm->mode) {
        case EC_FSM_SII_USE_INCREMENT_ADDRESS:
            ec_datagram_aprd(datagram, fsm->slave->ring_position, 0x502, data_size);
            break;
        case EC_FSM_SII_USE_CONFIGURED_ADDRESS:
            ec_datagram_fprd(datagram, fsm->slave->station_address, 0x502, data_size);
            break;
    }
#endif
        ...
        )
{
    ...
    {
        int data_size = (fsm->length > 4) ? 8 : 4;
#ifdef CONFIG_USE_LIB_MEMCPY
        memcpy(fsm->value, datagram->data + 6, data_size);
#else
        ec_memcpy(fsm->value, datagram->data + 6, data_size);
#endif
    }
        ...
        )
{
    ...
    EC_WRITE_U16(datagram->data + 2, fsm->word_offset);
    ec_memset(datagram->data + 4, 0x00, 2);
#ifdef CONFIG_USE_LIB_MEMCPY
	memcpy(datagram->data + 6, fsm->value, 2);
#else
	ec_memcpy(datagram->data + 6, fsm->value, 2);
#endif
#ifdef SII_DEBUG
