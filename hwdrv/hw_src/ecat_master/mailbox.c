...
Description:  EtherCAT Mailbox access implementation

Others:

Function List:  
                    ec_mbox_init: mailox structures init                   
                    ec_fsm_soe_clear: mailox structures clear 
                    ec_slave_mbox_queue_datagrams: Add mailbox(to specifc slave) datagram to Master's send queue
                    ec_master_mbox_queue_datagrams:Add mailbox datagram to Master's send queue
                    ec_slave_mbox_prepare_send: Generate mbox datagram
                    ec_slave_mbox_prepare_check: Generate datagram for checking mbox access result status 
                    ec_slave_mbox_check: Check SyncM state
                    ec_slave_mbox_prepare_fetch: Generate datagram to get mailbox data
                    ec_slave_mbox_fetch: Fetch the mailox data
...
/******************************************************************************
 *
 *  $Id: mailbox.c,v 1.1 2015/02/06 05:23:05 kidd Exp $
 *
    ...
    if (*type == 0x00) {
        ...
        for (mbox_msg = mbox_error_messages; mbox_msg->code; mbox_msg++) {
            if (mbox_msg->code != code)
                continue;
            _DBG_PRINTF_("Code 0x%04X: \"%s\".\n",
                    mbox_msg->code, mbox_msg->message);
            break;
        }

        if (!mbox_msg->code)
            _DBG_PRINTF_("Unknown error reply code 0x%04X.\n", code);
