...
Description:  FMMU Config

Others:

Function List:  
                    ec_fmmu_config_init      :  FMMU config info init
                    ec_fmmu_config_domain: Attach one FMMU config with domain
                    ec_fmmu_config_page    :Generate one FMMU config pageg
History:    
...
/******************************************************************************
 *
 *  $Id: fmmu_config.c,v 1.1 2015/02/06 05:23:04 kidd Exp $
 *
...
#else
	uart_printf("FMMU: LogAddr 0x%08X, DomAddr 0x%08X,"
			" Size %3u, Tx %3u"
			" PhysAddr 0x%04X, SM%u, Dir %s\n",
			fmmu->logical_start_address, fmmu->domain_address,
			fmmu->data_size, fmmu->data_size,
...
#else
	EC_WRITE_U16(data + 8,	fmmu_phy_start_addr(fmmu->sc->slave, sync->physical_start_address));
#endif
