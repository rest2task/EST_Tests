...
Description:  PDO list process
Others:

Function List:  
                    ec_pdo_list_init: PDO list init                  
                    ec_pdo_list_clear: PDO list clear
...
/******************************************************************************
 *
 *  $Id: pdo_list.c,v 1.7 2015/02/28 01:09:50 kidd Exp $
 *
...
void ec_pdo_list_clear_pdos(ec_pdo_list_t *pl /**< PDO list. */)
{
    ...
    list_for_each_entry_safe(pdo,ec_pdo_t, next, &pl->list, list) {
        ...
        )
{
	...
	_DBG_PRINTF_("%d in %s, ec_malloc size=%d\n", __LINE__, __FILE__, sizeof(ec_pdo_t));
#endif
    if (!(mapped_pdo = ec_malloc(sizeof(ec_pdo_t), 0))) {
        EC_ERR("Failed to allocate PDO memory.\n");
        return -ENOMEM;
    }
#else
	mapped_pdo = pdo_pool_get();
	if (mapped_pdo == NULL) {
		EC_ERR("Failed to allocate PDO memory.\n");
		return -ENOMEM;
	}
