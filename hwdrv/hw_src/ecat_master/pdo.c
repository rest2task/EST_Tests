...
Description:  PDO process
Others:

Function List:  
                    ec_pdo_init: PDO process sturctures init                  
                    ec_pdo_init_copy: copy PDO structure
                    ec_pdo_clear: Clear PDO structure
                    ec_pdo_clear_entries: Clear entries in PDO
                    ec_pdo_set_name: Set name to PDO
                    ec_pdo_add_entry: Add entry to PDO
                    ec_pdo_copy_entries: PDO entries copy
                    ec_pdo_equal_entries: PDO entries compare and return equal status
                    ec_pdo_entry_count: return the number of entry
                    ec_pdo_find_entry_by_pos_const: Return the entry with position
                    ec_pdo_print_entries: Print the entry info in PDO
...
/******************************************************************************
 *
 *  $Id: pdo.c,v 1.9 2015/02/28 01:09:50 kidd Exp $
 *
...
void ec_pdo_clear_entries(ec_pdo_t *pdo /**< EtherCAT PDO. */)
{
    ...
    list_for_each_entry_safe(entry,ec_pdo_entry_t, next, &pdo->entries, list) {
        list_del(&entry->list);

#ifdef DYN_MEM_ALLOC //gonggw, 2015.02.15, Remove dynamic memory allocation
        ...
        )
{
...
#endif
        if (!(pdo->name = (char *) ec_malloc(len + 1, 0))) {
	...
	len = strlen(name);
	if (name && (len != NULL)) {
		if (len > (PDO_NAME_LEN - 1))
		{
			memcpy(pdo->name, name, (PDO_NAME_LEN - 1));
			pdo->name[PDO_NAME_LEN - 1] = '\0';
		}
		else
		{
			memcpy(pdo->name, name, len + 1);			
...
#endif
    if (!(entry = ec_malloc(sizeof(ec_pdo_entry_t), 0))) {
